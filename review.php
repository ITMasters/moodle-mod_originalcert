...
/**
 * This page reviews a originalcert
 *
...
$PAGE->set_url('/mod/originalcert/review.php', array('id' => $cm->id));
...
if (!$certrecord = $DB->get_record('originalcert_issues', array('userid' => $USER->id, 'originalcertid' => $originalcert->id))) {
    notice(get_string('nooriginalcertsissued', 'originalcert'), "$CFG->wwwroot/course/view.php?id=$course->id");
    die;
}

// Load the specific originalcerttype
...
$reviewurl = new moodle_url('/mod/originalcert/review.php', array('id' => $cm->id));
...
echo html_writer::tag('p', get_string('viewed', 'originalcert'). '<br />' . userdate($certrecord->timecreated), array('style' => 'text-align:center'));

$link = new moodle_url('/mod/originalcert/review.php?id='.$cm->id.'&action=get');
$linkname = get_string('revieworiginalcert', 'originalcert');
