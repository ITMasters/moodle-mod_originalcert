...
/**
 * letter_embedded originalcert type
 *
...
originalcert_print_text($pdf, $x, $y + 55, 'C', $fontserif, '', 20, get_string('certify', 'originalcert'));
originalcert_print_text($pdf, $x, $y + 105, 'C', $fontserif, '', 30, fullname($USER));
originalcert_print_text($pdf, $x, $y + 155, 'C', $fontserif, '', 20, get_string('statement', 'originalcert'));
originalcert_print_text($pdf, $x, $y + 205, 'C', $fontserif, '', 20, format_string($course->fullname));
originalcert_print_text($pdf, $x, $y + 255, 'C', $fontserif, '', 14, originalcert_get_date($originalcert, $certrecord, $course));
originalcert_print_text($pdf, $x, $y + 283, 'C', $fontserif, '', 10, originalcert_get_grade($originalcert, $course));
originalcert_print_text($pdf, $x, $y + 311, 'C', $fontserif, '', 10, originalcert_get_outcome($originalcert, $course));
if ($originalcert->printhours) {
    originalcert_print_text($pdf, $x, $y + 339, 'C', $fontserif, '', 10, get_string('credithours', 'originalcert') . ': ' . $originalcert->printhours);
}
...
if ($originalcert->printteacher) {
    ...
    if ($teachers = get_users_by_capability($context, 'mod/originalcert:printteacher', '', $sort = 'u.lastname ASC', '', '', '', '', false)) {
        foreach ($teachers as $teacher) {
            $i++;
            originalcert_print_text($pdf, $sigx, $sigy + ($i * 12), 'L', $fontserif, '', 12, fullname($teacher));
        }
    }
}
